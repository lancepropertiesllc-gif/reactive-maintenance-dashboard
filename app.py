...
risk_df['Projected Emergency Cost'] = risk_df.apply(
    lambda row: row['Replacement Cost Estimate'] * 1.1 if pd.notna(row['Replacement Cost Estimate']) and row['Replacement Cost Estimate'] > 0 else row['Total Cost'] * 1.45, 
    axis=1
).round(0)
...
with colB:
    ...
    fig2 = px.scatter(filtered_df[filtered_df['Age_Years'] >= 7], x='Age_Years', y='Repair_Cost', color='Issue Type', size='Repair_Cost')
