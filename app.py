...
risk_df.columns = ['Max Age', 'Avg Cost', 'Total Cost', 'Breakdowns']
...
# New Total Estimated Emergency Cost
total_est_emergency = risk_df['Est Emergency Cost If It Breaks'].sum()

col5.metric("**Total Est. Emergency Costs**", f"${total_est_emergency:,.0f}", help="Projected surprise cost if high-risk items fail")
...
if not high_risk.empty:
    st.dataframe(high_risk[['Property', 'Issue_Type', 'Max Age', 'Failure Probability Next 12mo', 'Est Emergency Cost If It Breaks']])
