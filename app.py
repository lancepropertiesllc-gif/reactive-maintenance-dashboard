...
# Risk Calculation with Replacement Cost Logic
risk_df = filtered_df.groupby(['Property', 'Issue_Type']).agg({
    'Age_Years': 'max',
    'Repair_Cost': ['mean', 'sum', 'count'],
    'Unit_Count': 'first',
    'Replacement_Cost_Estimate': 'max'        # Use highest replacement estimate if multiple
}).round(1)
...
# Use Replacement Cost Estimate when available, otherwise inflate Repair Cost
risk_df['Projected Emergency Cost'] = risk_df.apply(
    lambda row: row['Replacement Cost Estimate'] * 1.1 if pd.notna(row['Replacement Cost Estimate']) and row['Replacement Cost Estimate'] > 0 
    else row['Total Cost'] * 1.45, axis=1
).round(0)
...
Replacement cost estimates (when provided) are used for more accurate projections on major capital items such as roofs and HVAC systems.
""")
