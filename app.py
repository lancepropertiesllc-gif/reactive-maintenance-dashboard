...
if uploaded_file is not None:
    ...
    if 'Issue_Type' in df.columns:
        df = df.rename(columns={'Issue_Type': 'Issue Type'})
    elif 'Issue Type' not in df.columns:
        st.error("CSV must have an 'Issue_Type' column. Please use the sample template.")
        st.stop()
...
selected_properties = st.sidebar.multiselect("Select Properties", df['Property'].unique(), default=df['Property'].unique()[:12])
...
# Risk Calculation
risk_df = filtered_df.groupby(['Property', 'Issue Type']).agg({
    'Age_Years': 'max',
...
total_est = risk_df['Projected Emergency Cost'].sum()
col5.metric("**Total Est. Emergency Costs**", f"${total_est:,.0f}")
...
with colA:
    ...
    fig1 = px.bar(filtered_df, x='Issue Type', y='Repair_Cost', color='Issue Type')
...
with colB:
    ...
    fig2 = px.scatter(filtered_df[filtered_df['Age_Years'] >= 7], 
                     ...
                     color='Issue Type', size='Repair_Cost')
    st.plotly_chart(fig2, use_container_width=True)

# ==================== HIGH RISK TABLE ====================
st.subheader("🚨 Highest Priority Items (Failure Probability > 55%)")
high_risk = risk_df[risk_df['Failure Probability Next 12mo'] > 55]

if not high_risk.empty:
    st.dataframe(high_risk[['Property', 'Issue Type', 'Max Age', 'Unit Count', 'Cost per Unit', 
                           'Failure Probability Next 12mo', 'Projected Emergency Cost']], 
                 use_container_width=True)
else:
    st.success("No extremely high-risk items detected at this time.")

st.subheader("Key Insights")
st.info("""
This analysis highlights items with elevated failure probability based on age. 
When a Replacement Cost Estimate is provided, it is used for more accurate projections on major capital items.
""")
